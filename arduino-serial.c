...
static void stop_tail(int signal) {
   run = 0;
}
...
int mainCom(int argc, char *argv[]) {
	...
	(void) signal (SIGINT, stop_tail);
	do {
		opt = getopt_long (argc, argv, "hp:b:s:S:d:qe:t:rTFo:",
		...
		case 'r':
			...
			if (run) {
				printf("Read until break with Ctrl-C\n");
				while(run) {
					serialport_read_until(fd, buf, eolchar, buf_max, timeout);
					if( !quiet ) {
						now = time(NULL);
						char buf[80];
						strftime(buf, sizeof(buf), "T:%Y-%m-%d %H:%M:%S", localtime(&now));
						printf("%s",buf);
					}
					printf(",F:%s", buf);
				}
